...
class shortenApp(webapp.webApp):
    ...
    def initializeDicts(self):
        ...
        try:
            ...
            print('Error: el diccionario está dañado. Comprueba ' +
                  'que las cabeceras de CSV estén colocadas al principio')
    ...
    def __init__(self, hostname, port, filename):
        ...
        self.form = ('<form method="post">' +
                     ...
                     '</form>')
        # El navegador dará los datos por POST a la misma página

        self.notFound = '<html><head><b>Error</b><br>Recurso no disponible' +
        '</head></html>'

        self.emptyRequest = ('<html><head><b>Error</b><br>' +
                             'Has enviado una URL vacia.<br>' +
                             '<a href="http://localhost:1234">' +
                             'Vuelve a intentarlo</a></head></html>')

        self.initializeDicts()

        self.lastIndex = len(self.shortenedURLs)
        # para no machacar las ya existentes

        webapp.webApp.__init__(self, hostname, port)
        # método init de la clase antigua
    ...
    def parse(self, request):
        ...
        if len(request) > 0:
            ...
            if verb == 'POST':
                # parse también va a buscar la URL en caso de ser un POST
            ...
            # firefox envía una petición vacía,
            # hay que tratarla para que no se caiga
            return ('POST', '/', None)
            # lo redirigimos a una página que process tratará como 404

    def process(self, parsedRequest):

        (verb, resource, newOne) = parsedRequest
        # para no violar mucho la sintaxis del paquete webapp
        ...
        if verb == 'GET':
            if resource == '/':
                ...
                if len(self.shortenedURLs) != 0:
                    # Imprimo la lista de las URLs ya acortadas (funciona)
                    responseBody = responseBody +
                    '<body><b><u>Listado de las URLs que ya' +
                    ' han sido acortadas:</u></b>'
                    for longOne in self.shortenedURLs:
                        responseBody = (responseBody + '<br>' +
                                        ...
                                        str(self.shortenedURLs[longOne]) +
                                        '</a>"')
                ...
                if resource in self.longURLs:
                    # si la acortada está en la lista:
                    returnCode = ('301 Moved Permanently\r\n' +
                                   'Location: ' + self.longURLs[resource])

                    responseBody = '<html><head><b>Redirecting...' +
                                    '</b></head></html>'
            ...
            if newOne is None:
                ...
                if newOne in self.shortenedURLs:
                    newIndex = self.shortenedURLs[newOne]
                    # la página ya existía. La vuelvo a servir
                else:
                    newURL = ('http://localhost:1234/' +
                    str(self.lastIndex))  # conversión para tener la barra
                    ...
                    responseBody = ('<html><head>Acortamiento realizado<br>' +
                                    '</head><body>' + '<a href="' +
                                    self.longURLs[newURL] +
                                    '">URL larga</a><br>' + '<a href="' +
                                    self.shortenedURLs[newOne] +
                                    '">URL corta</a>' + '</body></html>')
